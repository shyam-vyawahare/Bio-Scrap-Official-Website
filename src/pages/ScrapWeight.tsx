...
import { Scale, Check, Recycle } from "lucide-react";
            ...
            {scrapWeights.map((w) => (
              ...
              >
                <input
                  ...
                  onChange={() => setSelectedWeight(w.id)}
                  className="sr-only"
                />
                <Scale className={cn("w-8 h-8 mx-auto mb-2", selectedWeight === w.id ? "text-primary" : "text-muted-foreground")} />
                <h4 className="font-semibold text-foreground">{w.label}</h4>
              </label>
            ))}
          </div>

          <div className="flex justify-between mt-12 pt-8 border-t border-border">
            <Button type="button" variant="outline" onClick={() => navigate("/booking/scrap")}>
